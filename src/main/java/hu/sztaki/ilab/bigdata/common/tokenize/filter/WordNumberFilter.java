 ...
 * @author garzo
 */
public class WordNumberFilter extends IdentityTokenFilter {

    public WordNumberFilter(ITokenStream stream) {
    ...
    public Token next() {
        if (inputStream != null) {
            ...
            while (token != null && (token.getType() != TokenType.NUMBER && token.getType() != TokenType.WORD));
            return token;
        }
        else {
            return null;
